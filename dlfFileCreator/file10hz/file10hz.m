...
%     newData(i,7) = (newData(i,7)/2048);
    ...
    newData(i,5) = movmean(newData(i,5),5);
    newData(i,6) = movmean(newData(i,6),5);
    newData(i,7) = movmean(newData(i,7),5);
end
...
fileID = fopen(strcat(strtok(filename,'.'),'_movmean5.dlf'), 'wt')

dlmwrite(strcat(strtok(filename,'.'),'_movmean5.dlf'), newData,'delimiter','A','precision',9)
